"use client";

import { useEffect, useState } from "react";

function WormLoader() {
  return (
    <>
      <style>{`
        .pl-worm-loader {
          animation: bump9 3s linear infinite;
          width: 8em;
          height: 8em;
        }
        .pl-worm {
          animation: worm9 3s cubic-bezier(0.42, 0.17, 0.75, 0.83) infinite;
        }
        @keyframes bump9 {
          from, 42%, 46%, 51%, 55%, 59%, 63%, 67%, 71%, 74%, 78%, 81%, 85%, 88%, 92%, to {
            transform: translate(0, 0);
          }
          44% { transform: translate(1.33%, 6.75%); }
          53% { transform: translate(-16.67%, -0.54%); }
          61% { transform: translate(3.66%, -2.46%); }
          69% { transform: translate(-0.59%, 15.27%); }
          76% { transform: translate(-1.92%, -4.68%); }
          83% { transform: translate(9.38%, 0.96%); }
          90% { transform: translate(-4.55%, 1.98%); }
        }
        @keyframes worm9 {
          from { stroke-dashoffset: 10; }
          25%  { stroke-dashoffset: 295; }
          to   { stroke-dashoffset: 1165; }
        }
        .pl-worm-loader {
          filter: drop-shadow(0 0 10px hsla(0, 90%, 55%, 0.5))
                  drop-shadow(0 0 24px hsla(0, 90%, 55%, 0.25));
        }
        @media (prefers-reduced-motion: reduce) {
          .pl-worm-loader, .pl-worm { animation: none; }
        }
      `}</style>
      <svg
        className="pl-worm-loader"
        viewBox="0 0 128 128"
        xmlns="http://www.w3.org/2000/svg"
        role="status"
        aria-label="Loading"
      >
        <defs>
          <linearGradient id="pl-grad" x1="0" y1="0" x2="0" y2="1">
            {/* glass highlight — light pink-white at top */}
            <stop offset="0%"   stopColor="hsl(0,100%,82%)" />
            {/* vivid red-500 in the middle */}
            <stop offset="45%"  stopColor="hsl(0,84%,60%)"  />
            {/* deep crimson at the bottom */}
            <stop offset="100%" stopColor="hsl(0,72%,38%)"  />
          </linearGradient>
        </defs>
        <circle
          r="56" cx="64" cy="64"
          fill="none"
          stroke="hsla(0,84%,60%,0.12)"
          strokeWidth="16"
          strokeLinecap="round"
        />
        <path
          className="pl-worm"
          d="M92,15.492S78.194,4.967,66.743,16.887c-17.231,17.938-28.26,96.974-28.26,96.974L119.85,59.892l-99-31.588,57.528,89.832L97.8,19.349,13.636,88.51l89.012,16.015S81.908,38.332,66.1,22.337C50.114,6.156,36,15.492,36,15.492a56,56,0,1,0,56,0Z"
          fill="none"
          stroke="url(#pl-grad)"
          strokeWidth="16"
          strokeLinecap="round"
          strokeLinejoin="round"
          strokeDasharray="44 1111"
          strokeDashoffset="10"
        />
      </svg>
    </>
  );
}

export default function EmptyWithLoader() {
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const timer = setTimeout(() => setLoading(false), 2500);
    return () => clearTimeout(timer);
  }, []);

  return (
    <div className="flex flex-col items-center justify-center py-24 gap-6">
      {loading ? (
        <WormLoader />
      ) : (
        <div className="flex flex-col items-center gap-3">
          <h2 className="text-2xl font-semibold text-gray-700">
            No products found
          </h2>
          <p className="text-sm text-gray-400">
            This category is currently empty.
          </p>
        </div>
      )}
    </div>
  );
}