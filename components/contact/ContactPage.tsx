...
const validators: Record<FieldKey, (v: string) => boolean> = {
  name: v => v.trim().length >= 2 && /^[\u0600-\u06FFa-zA-Z\s]+$/.test(v.trim()),
...
function Field({ id, fieldKey, value, onChange, onBlur, state, placeholder, multiline, messages }: {
  id: string
  fieldKey: FieldKey
  value: string
  ...
  placeholder: string
  multiline?: boolean
  messages: { empty: string; error: string; ok: string }
}) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
      ...
      {(isError || isValid) && (
        <span style={{ fontSize: "11px", color: isError ? "#e53935" : "#43a047" }}>
          {isError ? (state === "empty" ? messages.empty : messages.error) : messages.ok}
        </span>
      )}
    </div>
  )
...
export default function ContactPage() {
  ...
  const validateField = useCallback((key: FieldKey, val: string): FieldState => {
    if (!val.trim()) return "empty"
    if (!validators[key](val)) return "error"
    return "valid"
  }, [])
  ...
  const addToast = useCallback((toast: Toast) => {
    setToasts(prev => {
      const filtered = prev.filter(t2 => t2.id !== toast.id)
  ...
  const handleSubmit = async () => {
    ...
    if (invalid.length === 0) {
      ...
      setIsLoading(false)
      addToast({ id: "success", type: "success", title: t.contact.toast.successTitle, sub: t.contact.toast.successSub, duration: 4000 })
    ...
    if (invalid.length === keys.length) {
      addToast({ id: "warn-all", type: "warn", title: t.contact.toast.emptyFormTitle, sub: t.contact.toast.emptyFormSub, duration: 3500 })
    } else {
      invalid.forEach(k => {
        const s = newStates[k]
        const fieldName = t.contact.fieldNames[k]
        const titleTemplate = s === "empty" ? t.contact.toast.fieldEmptyTitle : t.contact.toast.fieldErrorTitle
        addToast({
          id: "warn-" + k,
          type: "warn",
          title: titleTemplate.replace("{field}", fieldName),
          sub: t.contact.validation[k][s === "empty" ? "empty" : "error"],
          duration: 3000,
        })
      })
    }
  ...
  return (
    <section className="flex flex-col px-6 py-10 sm:px-8">
      <div style={{ position: "fixed", top: "20px", right: "20px", left: "20px", maxWidth: "360px", marginLeft: "auto", zIndex: 9999, display: "flex", flexDirection: "column", gap: "8px" }}>
        {toasts.map(toastItem => <ToastItem key={toastItem.id} toast={toastItem} onDismiss={removeToast} />)}
      </div>

      <div className="flex gap-2 text-sm text-gray-500 mb-10">
        <Link href="/" className="hover:text-red-500 transition">{t.breadcrumb.home}</Link>
        <span>/</span>
        <span className="text-black font-medium">{t.breadcrumb.contact}</span>
      </div>
        ...
        <div className="border border-gray-200 rounded-md shadow-sm p-5 flex flex-col gap-8 sm:p-8">
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-3">
              ...
              <h3 className="font-semibold text-base">{t.contact.callToUs}</h3>
            </div>
            <p className="text-sm text-gray-700">{t.contact.availability}</p>
            <p className="text-sm text-gray-700">{t.contact.phoneLabel}</p>
          </div>
          <hr />
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-3">
              ...
              <h3 className="font-semibold text-base">{t.contact.writeToUs}</h3>
            </div>
            <p className="text-sm text-gray-600">{t.contact.writeDesc}</p>
            <p className="text-sm text-gray-600">{t.contact.email1}</p>
            <p className="text-sm text-gray-600">{t.contact.email2}</p>
          </div>
        </div>

        <div className="lg:col-span-2 flex flex-col gap-3 p-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            {(["name", "email", "phone"] as FieldKey[]).map(k => (
              <div key={k} id={"field-" + k}>
                <Field
                  id={"input-" + k} fieldKey={k} value={values[k]}
                  onChange={handleChange} onBlur={handleBlur} state={states[k]}
                  placeholder={t.contact.placeholders[k]}
                  messages={t.contact.validation[k]}
                />
              </div>
            ))}
          </div>
          ...
          <div id="field-msg">
            <Field
              ...
              placeholder={t.contact.placeholders.msg} multiline
              messages={t.contact.validation.msg}
            />
          </div>
